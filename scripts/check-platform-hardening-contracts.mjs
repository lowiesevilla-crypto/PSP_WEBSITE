...
assert(memberDashboardPage.includes("ActionIcon") && memberDashboardPage.includes("quick-action-icon") && memberDashboardPage.includes("member-balance-panel"), "Member Dashboard must use polished icon quick actions and the mobile-safe balance panel.");
...
assert(healthRoute.includes('chapterFundsVersion: "month-year-expense-ledger-v1"') && healthRoute.includes('publicFeedMediaVersion: "image-archive-v1"') && healthRoute.includes('generalUpdatesVersion: "public-archive-member-link-v1"') && healthRoute.includes('memberDashboardUiVersion: "icon-cards-mobile-v1"'), "Health markers for Chapter Funds, public media, General Updates, and Member Dashboard UI are missing.");
