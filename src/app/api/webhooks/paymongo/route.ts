...
export async function POST(request: Request) {
  ...
  if (!payment || payment.gateway !== "PAYMONGO") {
      ...
      if (!existingLedger) {
        await tx.memberLedgerEntry.create({
          data: {
            ...
            description: "PayMongo online payment",
            occurredAt: paidAt,
          },
        });
      }
  ...
  if (!result.duplicate) {
    await notifyUser({
      ...
      body: `Your PSP payment of ₱${payment.amount.toFixed(2)} has been confirmed. Receipt ${result.receiptNumber}.`,
      href: "/payments",
    });
  }
